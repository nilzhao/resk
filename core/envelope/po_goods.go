...
type RedEnvelopeGoods struct {
	...
	EnvelopeNo       string                `json:"envelopeNo" db:"envelope_no,uni"`          // 红包编号, 红包唯一标识
	EnvelopeType     services.EnvelopeType `json:"envelopeType" db:"envelope_type"`          // 红包类型：普通红包，碰运气红包
	Username         sql.NullString        `json:"username" db:"username"`                   // 用户名称
	UserId           string                `json:"userId" db:"user_id"`                      // 用户编号,  红包所属用户
	Blessing         sql.NullString        `json:"blessing" db:"blessing"`                   // 祝福语
	Amount           decimal.Decimal       `json:"amount" db:"amount"`                       // 红包总金额
	AmountOne        decimal.Decimal       `json:"amountOne" db:"amount_one"`                // 单个红包金额，碰运气红包无效
	Quantity         int                   `json:"quantity" db:"quantity"`                   // 红包总数量
	RemainAmount     decimal.Decimal       `json:"remainAmount" db:"remain_amount"`          // 红包剩余金额额
	RemainQuantity   int                   `json:"remainQuantity" db:"remain_quantity"`      // 红包剩余数量
	ExpiredAt        time.Time             `json:"expiredAt" db:"expired_at"`                // 过期时间
	Status           services.OrderStatus  `json:"status" db:"status"`                       // 红包状态：0红包初始化，1启用，2失效
	OrderType        services.OrderType    `json:"orderType" db:"order_type"`                // 订单类型：发布单、退款单
	PayStatus        services.PayStatus    `json:"payStatus" db:"pay_status"`                // 支付状态：未支付，支付中，已支付，支付失败
	...
	OriginEnvelopeNo string                `json:"originEnvelopeNo" db:"origin_envelope_no"` // 原先关联的红包编号
}
